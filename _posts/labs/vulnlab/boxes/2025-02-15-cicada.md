...
If we try creds, we receive message `STATUS_NOT_SUPPORTED`, meaning `NTLM` is not supported
```
...
Based on that we can create DNS record that points to our box and has the form described in blogs. Knowing that by default all authenticated users can create A record if it doesn't exist, let's create it
```
